...
function Ordercard({order}) {
    ...
    async function getArtistName() {
        try {
            const res = await fetch(`http://localhost:3000/users/user/${order?.artist_id}`);
    ...
    async function getArtwork() {
        try {
            const res = await fetch(`http://localhost:3000/artworks/artwork/order/${order?.artwork_id}`);
            const data = await res.json();
            setImage(data.image);
            setTitle(data.title);
            setPrice(data.price);
            const categoryRes = await fetch(`http://localhost:3000/categories/${data.category}`);
    ...
    async function removeOrderitem() {
        try {
            const res = await fetch(`http://localhost:3000/orders/orderitem/${order?.order_item_id}`, {
                method: 'DELETE',
                headers: {
